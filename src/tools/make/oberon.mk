...
# Defaults for configuration files generated by older versions.
BINDIR ?= $(INSTALLDIR)/bin
LIBDIR ?= $(INSTALLDIR)/lib

.PHONY: install uninstall install-system uninstall-system
...
# uninstall: Remove files installed by this makefile without changing the
#              host dynamic linker configuration.
uninstall:
	@rm -f "$(DESTDIR)$(BINDIR)/$(OBECOMP)" "$(DESTDIR)$(BINDIR)/showdef$(BINEXT)"
	@rm -rf "$(DESTDIR)$(INSTALLDIR)/2" "$(DESTDIR)$(INSTALLDIR)/C"
	@rm -f "$(DESTDIR)$(LIBDIR)/lib$(ONAME)-O2.a" "$(DESTDIR)$(LIBDIR)/lib$(ONAME)-O2$(DYNEXT)"
	@rm -f "$(DESTDIR)$(LIBDIR)/lib$(ONAME)-OC.a" "$(DESTDIR)$(LIBDIR)/lib$(ONAME)-OC$(DYNEXT)"


# install: Use only after a successful full build. DESTDIR is a staging
# prefix and is deliberately not included in the generated compiler paths.
install:
	@printf '\nInstalling compiler to "$(DESTDIR)$(BINDIR)"\n'
	@mkdir -p "$(DESTDIR)$(BINDIR)"
	@cp "$(ROOTDIR)/install/bin/$(OBECOMP)" "$(DESTDIR)$(BINDIR)/$(OBECOMP)"
	@-cp "$(ROOTDIR)/install/bin/showdef$(BINEXT)" "$(DESTDIR)$(BINDIR)"

	@mkdir -p "$(DESTDIR)$(INSTALLDIR)/2" "$(DESTDIR)$(INSTALLDIR)/C"
	@cp -rf "$(ROOTDIR)/install/2/." "$(DESTDIR)$(INSTALLDIR)/2/"
	@cp -rf "$(ROOTDIR)/install/C/." "$(DESTDIR)$(INSTALLDIR)/C/"

	@mkdir -p "$(DESTDIR)$(LIBDIR)"
	@cp "$(ROOTDIR)/install/lib/"* "$(DESTDIR)$(LIBDIR)/"
	@printf '\nOberon compiler installed.\n'


# install-system: Install and update the dynamic linker cache for a manual
# installation. Package managers should use install with DESTDIR instead.
install-system: install
	@sh src/tools/make/addlibrary.sh install "$(LIBDIR)" $(ONAME)

uninstall-system: uninstall
	@sh src/tools/make/addlibrary.sh uninstall "$(LIBDIR)" $(ONAME)
	@printf '\n'
...
ooc:
	...
	cd $(BUILDDIR)/$(MODEL); "$(ROOTDIR)/$(OBECOMP)" -Fs -O$(MODEL) ../../../src/library/ooc/oocProgramArgsHost.Mod
	cd $(BUILDDIR)/$(MODEL); "$(ROOTDIR)/$(OBECOMP)" -Fs -O$(MODEL) ../../../src/library/ooc/oocProgramArgs.Mod
	...
	cd $(BUILDDIR)/$(MODEL); "$(ROOTDIR)/$(OBECOMP)" -Fs -O$(MODEL) ../../../src/library/ooc/oocFilesHost.Mod
	cd $(BUILDDIR)/$(MODEL); "$(ROOTDIR)/$(OBECOMP)" -Fs -O$(MODEL) ../../../src/library/ooc/oocFiles.Mod
	cd $(BUILDDIR)/$(MODEL); "$(ROOTDIR)/$(OBECOMP)" -Fs -O$(MODEL) ../../../src/library/ooc/oocFilenames.Mod
